...
(* Assuming Ll.ty and Ll.fdecl are already defined as per your ll file *)

(* Function to build uid -> ty map from a function declaration *)
let build_type_map (f: Ll.fdecl) : Ll.ty UidM.t =
  let (entry, blocks) = f.f_cfg in

  (* Function to process a single instruction and update the map *)
  let process_insn (type_map: Ll.ty UidM.t) (uid, insn) =
    match insn with
    | Ll.Binop (_, t, _, _)
    | Ll.Load (t, _)
    | Ll.Icmp (_, t, _, _)
    ...
    | Ll.Bitcast (t, _, _)
    | Ll.Gep (t, _, _) ->
        UidM.add uid t type_map
    | Ll.Alloca _
    | Ll.Store _ ->
        type_map (* These instructions do not define a new uid with a type *)
  in

  (* Function to process a terminator - usually terminators do not define new uids *)
  let process_term (type_map: Ll.ty UidM.t) (_uid, term) =
    type_map (* No action needed for terminators *)
  in

  (* Function to process all instructions in a block *)
  let process_block (type_map: Ll.ty UidM.t) (block: Ll.block) =
    let type_map = List.fold_left process_insn type_map block.insns in
    let type_map = process_term type_map block.term in
    type_map
  in

  (* Initialize the map by processing the entry block and all other blocks *)
  let initial_map = UidM.empty in
  let initial_map = process_block initial_map entry in
  let initial_map = List.fold_left (fun acc (lbl, blk) -> process_block acc blk) initial_map blocks in
  initial_map
...
let fresh_uid () =
  incr counter;
  Printf.sprintf ".spilltmp%d" !counter

(* let ops_of_insn (i: Ll.insn) : Ll.operand list =
  match i with
  | Ll.Binop (_, _, o1, o2) -> [o1; o2]
  | Ll.Alloca _ -> []
  | Ll.Load (_, o) -> [o]
  | Ll.Store (_, o1, o2) -> [o1; o2]
  | Ll.Icmp (_, _, o1, o2) -> [o1; o2]
  | Ll.Call (_, o, os) -> o :: List.map snd os
  | Ll.Bitcast (_, o, _) -> [o]
  | Ll.Gep (_, o, os) -> o :: List.map snd os
 *)

 let insn_of_ops (i: Ll.insn) (new_ops: Ll.operand list) : Ll.insn =
  match i with
  | Ll.Binop (t, b, _, _) -> Ll.Binop (t, b, List.nth new_ops 0, List.nth new_ops 1)
  | Ll.Alloca _ -> i
  | Ll.Load (t, _) -> Ll.Load (t, List.hd new_ops)
  | Ll.Store (t, _, _) -> Ll.Store (t, List.nth new_ops 0, List.nth new_ops 1)
  | Ll.Icmp (t, c, _, _) -> Ll.Icmp (t, c, List.nth new_ops 0, List.nth new_ops 1)
  | Ll.Call (t, _, os) -> Ll.Call (t, List.hd new_ops, List.combine (List.map fst os) (List.tl new_ops))
  | Ll.Bitcast (t, _, t') -> Ll.Bitcast (t, List.hd new_ops, t')
  | Ll.Gep (t, _, os) -> Ll.Gep (t, List.hd new_ops, List.tl new_ops)


let terminator_of_ops (t: Ll.terminator) (new_ops: Ll.operand list) : Ll.terminator =
  match t with
  | Ll.Ret (t, None) -> Ll.Ret (t, None)
  | Ll.Ret (t, Some _) -> Ll.Ret (t, Some (List.hd new_ops))
  | Ll.Br l -> Ll.Br l
  | Ll.Cbr (o, l1, l2) -> Ll.Cbr (List.hd new_ops, l1, l2)

let rewrite_fdecl (f: Ll.fdecl) (spilled: UidSet.t) (* (type_map: uid -> ty)  *): Ll.fdecl =
  let (entry, bs) = f.f_cfg in

  let type_map = build_type_map f in

  (* 1. Create allocas for spilled vars at the start of the entry block *)
  let v_to_alloca = UidSet.fold (fun v acc ->
    let v_alloca = v ^ "_alloca" in
    UidM.add v v_alloca acc
  ) spilled UidM.empty in

  let allocas =
    UidM.fold (fun v v_alloca acc ->
      let v_ty = UidM.find v type_map in
      (v_alloca, Ll.Alloca v_ty)::acc
    ) v_to_alloca []
  in

  let rewrite_operand op =
    match op with
    | Ll.Id v when UidSet.mem v spilled ->
        let tmp_v = fresh_uid () in
        let v_alloca = UidM.find v v_to_alloca in
        let v_ty = UidM.find v type_map in
        (* Load from v_alloca into tmp_v *)
        [(tmp_v, Ll.Load(v_ty, Ll.Id v_alloca))], Ll.Id tmp_v
    | _ -> [], op
  in

  let rewrite_insn (uid, i) =
    let ops = Liveness.insn_ops i in
    let (before_loads, new_ops) = 
      List.fold_left (fun (lds, ops_acc) op ->
        let (new_lds, new_op) = rewrite_operand op in
        (lds @ new_lds, ops_acc @ [new_op])
      ) ([], []) ops
    in
    let new_i = insn_of_ops i new_ops in

    (* If this insn defines a spilled var, store it after *)
    let after_stores =
      if insn_assigns i && UidSet.mem uid spilled then
        let v_alloca = UidM.find uid v_to_alloca in
        let store_uid = fresh_uid () in
        let v_ty = UidM.find uid type_map in
        [(store_uid, Ll.Store(v_ty, Ll.Id uid, Ll.Id v_alloca))]
      else
        []
    in

    before_loads @ [(uid, new_i)] @ after_stores
  in

  let rewrite_term (uid, t) =
    let ops = Liveness.terminator_ops t in
    let (before_loads, new_ops) =
      List.fold_left (fun (lds, ops_acc) op ->
        let (new_lds, new_op) = rewrite_operand op in
        (lds @ new_lds, ops_acc @ [new_op])
    ...
    in
    let new_t = terminator_of_ops t new_ops in
    before_loads, (uid, new_t)
  in

  let rewrite_block (b:Ll.block) =
    let new_insns = List.concat (List.map rewrite_insn b.insns) in
    let (term_loads, new_term) = rewrite_term b.term in
    { insns = new_insns @ term_loads; term = new_term }
  in

  let new_entry = { entry with insns = allocas @ entry.insns } in
  let new_entry = rewrite_block new_entry in
  let new_bs = List.map (fun (l,b) -> (l, rewrite_block b)) bs in
  { f with f_cfg = (new_entry, new_bs) }



let rec better_layout (f:Ll.fdecl) (live:Liveness.liveness) : layout =
  (* ---------------------------------------------
     Build the interference graph
     --------------------------------------------- *)
  let ig = interference_graph f live in

  (* ---------------------------------------------
     Determine the set of available registers:
     We exclude Rax and Rcx because the code generator
     uses them. Use caller_save minus {Rax,Rcx}.
     --------------------------------------------- *)
  let pal = LocSet.(caller_save
                    |> remove (Alloc.LReg X86.Rax)
                    |> remove (Alloc.LReg X86.Rcx)) in
  let allah = LocSet.cardinal pal in

  (* ---------------------------------------------
     Simplify/Spill Algorithm (Chaitin-Briggs style):
     We'll repeatedly remove nodes of low degree.
     If we can't, we spill one node.
     --------------------------------------------- *)
  let worklist = ref ig in
  let stack = ref [] in
  let spilled = ref UidSet.empty in

  let choose_spill_candidate ig =
    (* A simple heuristic: pick the first node *)
    fst (UidM.choose ig)
  in

  (* TODO: consider precoloring *)
  let rec simplify () =
    (* Partition nodes into low-degree and others *)
    let low_degree, others =
      UidM.partition (fun _ neighs -> UidSet.cardinal neighs < allah) !worklist
    in
    if not (UidM.is_empty low_degree) then begin
      (* Move these low-degree nodes to the stack and remove from graph *)
      UidM.iter (fun n _ ->
        stack := n :: !stack;
        worklist := UidM.remove n !worklist;
        (* Remove edges related to n *)
        worklist := UidM.map (fun neighs -> UidSet.remove n neighs) !worklist;
      ) low_degree;
      simplify ()
    end else if not (UidM.is_empty !worklist) then begin
      (* Spill: pick a node to spill *)
      let n = choose_spill_candidate !worklist in
      spilled := UidSet.add n !spilled;
      stack := n :: !stack;
      worklist := UidM.remove n !worklist;
      worklist := UidM.map (fun neighs -> UidSet.remove n neighs) !worklist;
      simplify ()
    end
    (* else: no nodes left, we're done *)
  in
  ...
  while !stack <> [] do
    ...
    stack := List.tl !stack;
    if UidSet.mem n !spilled then
      (* Spilled node: no register *)
      Hashtbl.add color n None
    else begin
      let neighs = try UidM.find n ig with Not_found -> UidSet.empty in
      let used =
        UidSet.fold (fun nb used ->
          match Hashtbl.find_opt color nb with
          | Some (Some r) -> LocSet.add r used
          | _ -> used
        ) neighs LocSet.empty
      in
      let avail = LocSet.diff pal used in
      if LocSet.is_empty avail then
        (* No register available, spill *)
        Hashtbl.add color n None
      else
        (* Choose an arbitrary available register *)
        Hashtbl.add color n (Some (LocSet.choose avail))
    end
  done;
