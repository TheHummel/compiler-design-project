...
let index_into tdecls (ts:ty list) (n:int) : int * ty =
  let rec loop ts n acc =
    begin match (ts, n) with
      ...
      | _ -> failwith "index_into encered bogus index"
    end
  in loop ts n 0
...
let compile_getelementptr tdecls (t:Ll.ty) (o:Alloc.operand)
  ...
  let rec loop ty path (code : x86stream) =
    ...
    | _ -> failwith "compile_gep encered unsupported getelementptr data" in
...
type node = (UidSet.t * Alloc.loc option * int)
type graph = node UidMap.t
  
let build_graph (f:Ll.fdecl) (live:liveness) : graph =
  
  let rec collect_ids ops =
    match ops with
    | [] -> []
    | Id(uid) :: rest -> uid :: collect_ids rest
    | _ :: rest -> collect_ids rest
  in

  let init_labels (graph:graph) =
    let labels = UidSet.of_list (List.fold_left (fun acc (lbl, _) -> lbl :: acc) [] (snd f.f_cfg)) in
    UidSet.fold (fun lbl g ->
      UidMap.add lbl (UidSet.empty, Some(Alloc.LLbl(Platform.mangle lbl)), 0) g
    ) labels graph
  in
  
  let ins_ops (i: Ll.insn) : UidSet.t =
    let ops = match i with
      | Binop(_, _, op1, op2) -> [op1; op2]
      | Load(_, op) -> [op]
      | Store(_, op1, op2) -> [op1; op2]
      | Icmp(_, _, op1, op2) -> [op1; op2]
      | Call(_, fn_op, fn_args) -> fn_op :: (List.map snd fn_args)
      | Bitcast(_, op, _) -> [op]
      | Gep(_, base_op, idx_ops) -> base_op :: idx_ops
      | _ -> []
    in
    collect_ids ops |> UidSet.of_list
  
  in

  let term_ops (t:Ll.terminator) : UidSet.t =
    match t with
    | Ret(_, Some(Id(uid))) -> UidSet.singleton uid
    | Cbr(Id(uid), _, _) -> UidSet.singleton uid
    | _ -> UidSet.empty
  in


  let init_params (graph:graph) : graph =
    ...
    in
    let (updated_graph, _) =
      List.fold_left (fun (g, regs) p ->
        let interferences = UidSet.remove p params in
        match regs with
        | [] -> (UidMap.add p (interferences, None, 1) g, [])
        | reg::rest -> (UidMap.add p (interferences, reg, 4) g, rest)
      ) (graph, param_regs) f.f_param
    in
    updated_graph
  in

  let process_ins ((uid, insn) : uid * Ll.insn) (graph:graph) : graph =
    let rec new_graph uids graph =
      match UidSet.choose_opt uids with
      | Some u ->
          let graph =
            match UidMap.find_opt u graph with
            | Some (neighbors, loc, c) -> UidMap.add u (neighbors, loc, c + 1) graph
            | None -> UidMap.add u (UidSet.empty, None, 1) graph
          in
          new_graph (UidSet.remove u uids) graph
      | None -> graph
    in
    let graph = new_graph (ins_ops insn) graph in
    

  let update_dep uid dep (graph, dep_set) =
    match UidMap.find_opt dep graph with
    | Some (neighbors, loc, c) ->
      (match loc with
        | Some Alloc.LVoid | Some Alloc.LLbl(_) -> (graph, dep_set)
        | _ ->
          let new_neighbors = UidSet.add uid neighbors in
          (UidMap.add dep (new_neighbors, loc, c) graph, UidSet.add dep dep_set))
    | None ->
      let new_neighbors = UidSet.singleton uid in
      (UidMap.add dep (new_neighbors, None, 0) graph, UidSet.add dep dep_set)
      in
    
    let new_graph_dep live_out graph uid =
      UidSet.fold (fun dep acc -> update_dep uid dep acc) live_out (graph, UidSet.empty)
    in
    
    if insn_assigns insn then
      let live_out = UidSet.remove uid (live.live_out uid) in
      let (graph', neighbors2) = new_graph_dep live_out graph uid in
      let final_graph =
        if UidMap.mem uid graph' then
          UidMap.update
            (fun (old_neighbors, old_loc, old_c) ->
              let all_neighbors = UidSet.union old_neighbors neighbors2 in
              (all_neighbors, old_loc, old_c + 1))
            uid graph'
        else
          UidMap.add uid (neighbors2, None, 0) graph'
      in
      final_graph
    else
      UidMap.add uid (UidSet.empty, Some Alloc.LVoid, 0) graph

    in
    
  let process_block (block:Ll.block) (graph:graph) : graph =
    let graph =
      UidSet.fold (fun u g ->
        match UidMap.find_opt u g with
        | Some(neighbors, loc, c) -> UidMap.add u (neighbors, loc, c + 1) g
        | None -> UidMap.add u (UidSet.empty, None, 1) g
      ) (term_ops (snd block.term)) graph
    in
    List.fold_left (fun g i -> process_ins i g) graph block.insns
  in

  let (block, blocks) = f.f_cfg in
  let graph = init_labels UidMap.empty in
  let graph = init_params graph in
  let graph = process_block block graph in
  List.fold_left (fun g (_, b) -> process_block b g) graph blocks

let build_graph_opt (f: Ll.fdecl) (live: liveness) : graph option =
  let graph = build_graph f live in
  Some graph


let alloc_regs (graph:graph) (available_regs:LocSet.t): (graph * UidSet.t) =

  let find_node (g:graph) (max:int) : UidSet.t =
    UidMap.fold (fun uid (neighbors, _, _) acc ->
      if UidSet.cardinal neighbors > max then UidSet.add uid acc else acc
    ) g UidSet.empty
  in

  let get_avail ((neighbors, _, _) : node) (g:graph) (avail:LocSet.t) : LocSet.t =
    UidSet.fold (fun u s ->
      match UidMap.find_opt u g with
      | Some(_, Some(loc), _) -> LocSet.remove loc s
      | _ -> s
    ) neighbors avail
  in

  let rem_node (node:uid) (g:graph) : graph =
    let g' = UidMap.remove node g in
    UidMap.map (fun (neighbors, loc, cnt) -> (UidSet.remove node neighbors, loc, cnt)) g'
  in

  let min (g: graph) (candidates: UidSet.t) : uid =
    let (choice, _) =
      UidSet.fold
        (fun u (choice, min_occ) ->
          let (_, _, occ) = UidMap.find u g in
          if occ < min_occ then (Some u, occ) else (choice, min_occ))
        candidates
        (None, max_int)
    in
    match choice with
  ...
  let assign_regs (node:uid) (g:graph) (avail:LocSet.t) : graph =
    let (neighbors, old_loc, occ) = UidMap.find node g in
    match old_loc with
    | None ->
      let avail = get_avail (neighbors, old_loc, occ) g avail in
      let chosen = LocSet.choose avail in
      UidMap.add node (neighbors, Some(chosen), occ) g
    | Some(_) -> g
  in

  let assign_all_regs (g:graph) (avail:LocSet.t) : graph =
    let all_nodes = UidMap.fold (fun uid _ acc -> uid :: acc) g [] in
    List.fold_left (fun acc uid -> assign_regs uid acc avail) g all_nodes
  in

  let color_graph (g: graph) (avail: LocSet.t) : (graph * UidSet.t) =
    let rec loop g spilled =
      let spill_cand = find_node g (LocSet.cardinal avail - 1) in
      if UidSet.is_empty spill_cand then
        (assign_all_regs g avail, spilled)
      else
        let node = min g spill_cand in
        let n_data = UidMap.find node g in
        let g = rem_node node g in
        let (g_colored, spilled) = loop g spilled in
        let new_avail = get_avail n_data g_colored avail in
        if LocSet.is_empty new_avail then
          (UidMap.add node n_data g_colored, UidSet.add node spilled)
        else
          let (neighbors, _, occ) = n_data in
          let assigned_data = (neighbors, Some (LocSet.choose new_avail), occ) in
          (UidMap.add node assigned_data g_colored, spilled)
    in
    loop g UidSet.empty
  in  
  let rec new_graph acc = function
  | [] -> acc
  | (uid, (neighbors, loc, occ)) :: rest ->
    let new_node =
      match (loc, occ) with
      | (None, 0) | (Some (Alloc.LReg _), 0) -> (neighbors, Some Alloc.LVoid, 0)
      | _ -> (neighbors, loc, occ)
    in
    new_graph (UidMap.add uid new_node acc) rest
  in
  let g = new_graph UidMap.empty (UidMap.bindings graph)
  in

  let filtered_graph =
    UidMap.fold (fun uid (neighbors, loc, occ) acc ->
      if occ <> 0 && (match loc with Some Alloc.LVoid | Some Alloc.LLbl(_) -> false | _ -> true)
      then UidMap.add uid (neighbors, loc, occ) acc
      else acc
    ) g UidMap.empty
  in  

  let (final_g, spills) = color_graph filtered_graph available_regs in
  let merged_g =
    UidMap.merge (fun _ original modified ->
      match (original, modified) with
      | (Some(o), _) -> Some(o)
      | (None, Some(m)) -> Some(m)
      | _ -> failwith "something stupid"
    ) final_g g
  in
  (merged_g, spills)
...
let better_layout (f:Ll.fdecl) (live:liveness) : layout =
  let initial_regs =
    LocSet.remove (Alloc.LReg Rcx) (LocSet.remove (Alloc.LReg Rax) caller_save)
  in  
  let graph =
    match build_graph_opt f live with
    | Some g -> g
    | None -> failwith "built failed"
  
  in
  let (final_graph, spilled) = alloc_regs graph initial_regs in
  let initial_map = UidMap.filter_map (fun _ (_, loc, _) -> loc) final_graph in  
  let add_spill uid (map, offset) =
    let stack_loc = Alloc.LStk(-(offset + 1)) in
    (UidMap.add uid stack_loc map, offset + 1)
  
  in
  let (loc_map, spill_c) = UidSet.fold add_spill spilled (initial_map, 0) in  

  let uid_loc (u: uid) : Alloc.loc =
    match UidMap.find_opt u loc_map with
    | Some loc -> loc
    | None -> failwith ("no uid")
  in
  

  { uid_loc = uid_loc; spill_bytes = spill_c * 8 }
